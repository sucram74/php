...
const commercialVisuals = [
  { title: 'Loja de roupas', src: 'https://source.unsplash.com/1200x800/?clothing-store,fashion-boutique' },
  { title: 'Loja de calçados', src: 'https://source.unsplash.com/1200x800/?shoe-store,sneakers-shop' },
  { title: 'Cliente comprando', src: 'https://source.unsplash.com/1200x800/?retail-customer,shopping' },
  { title: 'Campanha promocional', src: 'https://source.unsplash.com/1200x800/?marketing-campaign,retail-promo' },
  { title: 'Sorteio e cupons', src: 'https://source.unsplash.com/1200x800/?gift-card,coupon' },
];
    ...
    <main className='bg-slate-50 text-slate-900'>
      ...
      <section className='mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6 lg:px-8'>
        <div className='mb-8 flex items-end justify-between gap-4'>
          <div>
            <h2 className='text-3xl font-bold tracking-tight'>Cenários comerciais reais</h2>
            <p className='mt-2 max-w-2xl text-slate-600'>Visualize como o Promo SaaS se conecta com o dia a dia de lojas de moda, calçados e campanhas promocionais.</p>
          </div>
          <a href='#mockups' className='hidden rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 md:inline-block'>Ver produto em ação</a>
        </div>
        <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {commercialVisuals.map((item, idx) => (
            <article key={item.title} className={`overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm ${idx === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}`}>
              <img src={item.src} alt={item.title} loading='lazy' className='h-48 w-full object-cover sm:h-56' />
              <div className='border-t border-slate-100 p-4'>
                <h3 className='font-semibold text-slate-900'>{item.title}</h3>
                <p className='mt-1 text-sm text-slate-600'>Experiência comercial com foco em conversão, engajamento e recompra.</p>
              </div>
            </article>
          ))}
        </div>
