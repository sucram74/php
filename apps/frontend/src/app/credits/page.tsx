...
const statusPt = (s:string)=>({pending:'Pendente',pending_review:'Aguardando análise',paid:'Pago',rejected:'Rejeitado'} as any)[s]||s;
const typePt = (t:string)=>({purchase:'Compra',consumption:'Consumo'} as any)[t]||t;
...
export default function(){
  ...
  useEffect(()=>{if(b===1){setToast('Você possui apenas 1 crédito restante. Deseja adquirir mais créditos agora?');}},[b]);
  const baseUrl = process.env.NEXT_PUBLIC_FRONTEND_PUBLIC_URL || 'http://localhost:3000';
  return <div className='space-y-4'><Toast message={toast} onClose={()=>setToast('')}/><h1 className='text-2xl font-bold'>Créditos</h1><div className='ui-card flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'><span>Saldo atual: <strong>{b}</strong></span><Button onClick={()=>setOpen(true)}>Adicionar créditos</Button></div>
  {open && <div className='fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4'><div className='ui-card w-full max-w-xl space-y-3'><h2 className='text-lg font-semibold'>Comprar créditos</h2><select className='ui-input' value={selectedPackage} onChange={e=>setSelectedPackage(e.target.value)}><option value=''>Selecione um pacote</option>{p.map(x=><option key={x.id} value={x.id}>{x.name} — {x.quantity} créditos — {formatBRL(Number(x.price))}</option>)}</select><div className='flex flex-col gap-2 sm:flex-row'><Button variant='secondary' onClick={()=>setOpen(false)}>Cancelar</Button><Button disabled={loading||!selectedPackage} onClick={async()=>{setLoading(true);try{const created=await api('/credits/purchase',{method:'POST',body:JSON.stringify({packageId:selectedPackage})});setTx(created);setToast('Compra iniciada com sucesso.');setOpen(false);setSelectedPackage('');void load();}catch(e){setToast(e instanceof ApiError?e.message:'Erro ao comprar créditos.');}finally{setLoading(false);}}}>{loading?'Processando...':'Confirmar'}</Button></div></div></div>}
  {tx&&<div className='ui-card space-y-2'><p>Chave Pix: {tx.pixKey}</p><p>Pix copia e cola: {tx.pixCopyPaste}</p><Button onClick={async()=>{try{await api(`/credits/transactions/${tx.id}/inform`,{method:'PATCH'});setToast('Pagamento informado. Aguarde confirmação.');void load();}catch(e){setToast(e instanceof ApiError?e.message:'Erro ao informar pagamento.');}}}>Já paguei</Button><Button onClick={()=>{const msg=encodeURIComponent(`Olá, sou da loja ${store}. Realizei o pagamento do pacote ${tx.packageName}, no valor de ${formatBRL(Number(tx.amount))}, mas meus créditos ainda não foram liberados. Pode verificar, por favor?`); window.open(`https://wa.me/5500000000000?text=${msg}&ref=${encodeURIComponent(baseUrl)}`,'_blank')}}>Chamar suporte</Button></div>}
  <div className='ui-card'><h2 className='mb-2 font-semibold'>Histórico de movimentação</h2><div className='ui-table-wrap'><div className='min-w-[620px] space-y-2'>{history.map((h:any)=><div key={h.id} className='flex justify-between border-b pb-2 text-sm'><span>{typePt(h.type)} • {statusPt(h.status)}</span><span>{h.quantity} crédito(s) • {formatBRL(Number(h.amount))}</span></div>)}</div></div></div>
  </div>
}
