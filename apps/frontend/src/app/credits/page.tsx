...
export default function(){
  ...
  return <div className='space-y-4'><Toast message={toast} onClose={()=>setToast('')}/><h1 className='text-2xl font-bold'>Créditos</h1><div className='ui-card flex items-center justify-between'><span>Saldo atual: <strong>{b}</strong></span><Button onClick={()=>setOpen(true)}>Adicionar créditos</Button></div>
  ...
  <div className='ui-card'><h2 className='font-semibold mb-2'>Histórico de movimentação</h2><div className='space-y-2'>{history.map((h:any)=><div key={h.id} className='flex justify-between border-b pb-2 text-sm'><span>{typePt(h.type)} • {statusPt(h.status)}</span><span>{h.quantity} crédito(s) • {formatBRL(Number(h.amount))}</span></div>)}</div></div>
  </div>
}
